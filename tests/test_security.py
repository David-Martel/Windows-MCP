...
import windows_mcp.desktop.service as _service_module
from windows_mcp.desktop.service import Desktop, _get_shell_blocklist
...
def _reset_blocklist_cache():
    ...
    _service_module._shell_blocklist_patterns = None
...
class TestValidateUrl:
    """Unit tests for Desktop._validate_url static method."""
    ...
    def test_valid_https_url(self):
        ...
        with patch("socket.getaddrinfo") as mock_dns:
            ...
            Desktop._validate_url("https://example.com")  # no exception
    ...
    def test_valid_http_url(self):
        with patch("socket.getaddrinfo") as mock_dns:
            ...
            Desktop._validate_url("http://example.com")  # no exception

    def test_blocks_file_scheme(self):
        with pytest.raises(ValueError, match="scheme"):
            Desktop._validate_url("file:///etc/passwd")

    def test_blocks_ftp_scheme(self):
        with pytest.raises(ValueError, match="scheme"):
            Desktop._validate_url("ftp://example.com")

    def test_blocks_data_scheme(self):
        with pytest.raises(ValueError, match="scheme"):
            Desktop._validate_url("data:text/html,<h1>xss</h1>")

    def test_blocks_no_scheme(self):
        # "example.com" is parsed with an empty scheme by urlparse.
        with pytest.raises(ValueError, match="scheme"):
            Desktop._validate_url("example.com")

    def test_blocks_loopback_ip(self):
        with pytest.raises(ValueError, match="private/reserved"):
            Desktop._validate_url("http://127.0.0.1")

    def test_blocks_loopback_ipv6(self):
        with pytest.raises(ValueError, match="private/reserved"):
            Desktop._validate_url("http://[::1]")

    def test_blocks_private_10_range(self):
        with pytest.raises(ValueError, match="private/reserved"):
            Desktop._validate_url("http://10.0.0.1")

    def test_blocks_private_172_range(self):
        # 172.16.0.0/12 is RFC-1918 private.
        with pytest.raises(ValueError, match="private/reserved"):
            Desktop._validate_url("http://172.16.0.1")

    def test_blocks_private_192_range(self):
        with pytest.raises(ValueError, match="private/reserved"):
            Desktop._validate_url("http://192.168.1.1")

    def test_blocks_link_local(self):
        # 169.254.169.254 is the AWS/GCP/Azure instance metadata endpoint.
        with pytest.raises(ValueError, match="private/reserved"):
            Desktop._validate_url("http://169.254.169.254")

    def test_blocks_empty_hostname(self):
        with pytest.raises(ValueError, match="hostname"):
            Desktop._validate_url("http://")
    ...
    def test_blocks_domain_resolving_to_private_ip(self):
        ...
        with patch("socket.getaddrinfo") as mock_dns:
            ...
            with pytest.raises(ValueError, match="private/reserved"):
                Desktop._validate_url("https://evil-rebinding.example.com")
    ...
    def test_blocks_domain_with_any_private_address(self):
        ...
        with patch("socket.getaddrinfo") as mock_dns:
            ...
            with pytest.raises(ValueError, match="private/reserved"):
                Desktop._validate_url("https://mixed-records.example.com")
    ...
    def test_blocks_unresolvable_hostname(self):
        with patch("socket.getaddrinfo", side_effect=socket.gaierror("Name not found")):
            with pytest.raises(ValueError, match="Could not resolve hostname"):
                Desktop._validate_url("https://this-does-not-exist.invalid")
...
class TestShellBlocklist:
    ...
    def test_iex_downloadstring_blocked(self):
        ...
        result = Desktop._check_shell_blocklist(cmd)
    ...
    def test_safe_dir_listing_allowed(self):
        result = Desktop._check_shell_blocklist("dir")
        assert result is None, "Expected 'dir' to be allowed"

    def test_safe_echo_allowed(self):
        result = Desktop._check_shell_blocklist("echo hello")
    ...
    def test_case_insensitive_stop_computer(self):
        result = Desktop._check_shell_blocklist("stop-computer -force")
    ...
    def test_bcdedit_blocked(self):
        result = Desktop._check_shell_blocklist(
            "bcdedit /set {current} bootstatuspolicy ignoreallfailures"
        )
        assert result is not None, "Expected 'bcdedit' to be blocked"

    def test_reg_delete_hive_blocked(self):
        result = Desktop._check_shell_blocklist("reg delete HKLM\\System\\CurrentControlSet")
    ...
    def test_restart_computer_blocked(self):
        result = Desktop._check_shell_blocklist("Restart-Computer -Force")
    ...
    def test_env_var_override(self, fresh_blocklist):
        ...
        with patch.dict("os.environ", {"WINDOWS_MCP_SHELL_BLOCKLIST": custom_pattern}):
            ...
            result = Desktop._check_shell_blocklist("forbidden_cmd --run")
            ...
            result_format = Desktop._check_shell_blocklist("format C:")
    ...
    def test_env_var_multiple_patterns(self, fresh_blocklist):
        ...
        with patch.dict(
            ...
            assert Desktop._check_shell_blocklist("danger_a") is not None
            assert Desktop._check_shell_blocklist("danger_b") is not None
            assert Desktop._check_shell_blocklist("safe_cmd") is None
