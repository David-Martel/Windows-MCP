...
lock_screen, get_windows_version, get_user_account_type, get_default_language,
and get_state tree_state exception handling.
...
# ===========================================================================
# 6. get_windows_version
# ===========================================================================


class TestGetWindowsVersion:
    """get_windows_version reads from winreg, falls back to platform.release()."""

    def test_reads_product_name_from_registry(self):
        d = _make_bare_desktop()
        mock_key = MagicMock()
        with (
            patch("winreg.OpenKey", return_value=mock_key),
            patch("winreg.QueryValueEx", return_value=("Windows 11 Pro", 1)),
        ):
            mock_key.__enter__ = lambda s: s
            mock_key.__exit__ = MagicMock(return_value=False)
            result = d.get_windows_version()
    ...
    def test_fallback_format_is_windows_release(self):
        d = _make_bare_desktop()
        with (
            patch("winreg.OpenKey", side_effect=OSError),
            patch("platform.release", return_value="Server 2022"),
        ):
            result = d.get_windows_version()
        assert "Windows" in result
        assert "Server 2022" in result

    def test_returns_registry_value_not_platform(self):
        """When registry succeeds, platform.release() must NOT be called."""
        d = _make_bare_desktop()
        mock_key = MagicMock()
        mock_key.__enter__ = lambda s: s
        mock_key.__exit__ = MagicMock(return_value=False)
        with (
            patch("winreg.OpenKey", return_value=mock_key),
            patch("winreg.QueryValueEx", return_value=("Windows 10 Home", 1)),
            patch("platform.release") as mock_release,
        ):
            result = d.get_windows_version()
        mock_release.assert_not_called()
        assert result == "Windows 10 Home"

    def test_query_targets_correct_registry_path(self):
        d = _make_bare_desktop()
        import winreg as _winreg

        opened_paths = []

        def capture_open(hive, path, *args, **kwargs):
            opened_paths.append((hive, path))
            raise OSError("test stop")

        with (
            patch("winreg.OpenKey", side_effect=capture_open),
            patch("platform.release", return_value="10"),
        ):
            d.get_windows_version()

        assert len(opened_paths) == 1
        hive, path = opened_paths[0]
        assert hive == _winreg.HKEY_LOCAL_MACHINE
        assert "CurrentVersion" in path


# ===========================================================================
# 7. get_user_account_type
# ===========================================================================


class TestGetUserAccountType:
    """get_user_account_type returns 'Microsoft Account' for email-style names,
    'Local Account' otherwise, and 'Local Account' on OSError."""

    def _mock_registry_read(self, display_name: str):
        mock_key = MagicMock()
        mock_key.__enter__ = lambda s: s
        mock_key.__exit__ = MagicMock(return_value=False)
        return mock_key, display_name

    def test_email_name_returns_microsoft_account(self):
        d = _make_bare_desktop()
        mock_key = MagicMock()
        mock_key.__enter__ = lambda s: s
        mock_key.__exit__ = MagicMock(return_value=False)
        with (
            patch("winreg.OpenKey", return_value=mock_key),
            patch("winreg.QueryValueEx", return_value=("user@example.com", 1)),
        ):
            result = d.get_user_account_type()
        assert result == "Microsoft Account"

    def test_local_name_without_at_returns_local_account(self):
        d = _make_bare_desktop()
        mock_key = MagicMock()
        mock_key.__enter__ = lambda s: s
        mock_key.__exit__ = MagicMock(return_value=False)
        with (
            patch("winreg.OpenKey", return_value=mock_key),
            patch("winreg.QueryValueEx", return_value=("JohnDoe", 1)),
        ):
            result = d.get_user_account_type()
        assert result == "Local Account"

    def test_oserror_returns_local_account(self):
        d = _make_bare_desktop()
        with patch("winreg.OpenKey", side_effect=OSError("no key")):
            result = d.get_user_account_type()
        assert result == "Local Account"

    def test_at_sign_in_name_is_microsoft_account(self):
        d = _make_bare_desktop()
        mock_key = MagicMock()
        mock_key.__enter__ = lambda s: s
        mock_key.__exit__ = MagicMock(return_value=False)
        with (
            patch("winreg.OpenKey", return_value=mock_key),
            patch("winreg.QueryValueEx", return_value=("admin@corp.local", 1)),
        ):
            result = d.get_user_account_type()
        assert result == "Microsoft Account"

    def test_empty_name_without_at_is_local_account(self):
        d = _make_bare_desktop()
        mock_key = MagicMock()
        mock_key.__enter__ = lambda s: s
        mock_key.__exit__ = MagicMock(return_value=False)
        with (
            patch("winreg.OpenKey", return_value=mock_key),
            patch("winreg.QueryValueEx", return_value=("", 1)),
        ):
            result = d.get_user_account_type()
        assert result == "Local Account"

    def test_queries_correct_registry_key(self):
        d = _make_bare_desktop()
        import winreg as _winreg
        ...
        with patch("winreg.OpenKey", side_effect=capture_open):
            d.get_user_account_type()

        assert len(opened_paths) == 1
        hive, path = opened_paths[0]
        assert hive == _winreg.HKEY_LOCAL_MACHINE
        assert "LogonUI" in path

    def test_queries_correct_value_name(self):
        d = _make_bare_desktop()
        mock_key = MagicMock()
        mock_key.__enter__ = lambda s: s
        mock_key.__exit__ = MagicMock(return_value=False)
        ...
        def capture_query(key, name):
            queried_names.append(name)
            return ("LocalUser", 1)

        with (
            patch("winreg.OpenKey", return_value=mock_key),
            patch("winreg.QueryValueEx", side_effect=capture_query),
        ):
            d.get_user_account_type()

        assert "LastLoggedOnDisplayName" in queried_names


# ===========================================================================
# 8. get_default_language
# ===========================================================================


class TestGetDefaultLanguage:
    """get_default_language uses locale.getlocale() and replaces underscores."""

    def test_normal_locale_underscore_replaced(self):
        d = _make_bare_desktop()
        with patch("locale.getlocale", return_value=("en_US", "UTF-8")):
            result = d.get_default_language()
        assert result == "en US"
        assert "_" not in result

    def test_windows_style_locale(self):
        d = _make_bare_desktop()
    ...
    def test_none_locale_returns_unknown(self):
        d = _make_bare_desktop()
        with patch("locale.getlocale", return_value=(None, "UTF-8")):
            result = d.get_default_language()
        assert result == "Unknown"

    def test_exception_in_getlocale_returns_unknown(self):
        d = _make_bare_desktop()
        with patch("locale.getlocale", side_effect=ValueError("no locale")):
            result = d.get_default_language()
        assert result == "Unknown"
    ...
    def test_no_underscore_locale_unchanged(self):
        d = _make_bare_desktop()
...
# ===========================================================================
# 9. get_state -- tree_state exception handling
# ===========================================================================
